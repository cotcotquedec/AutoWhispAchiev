...
local AutoWhispAchivAddon = CreateFrame("Frame", "AutoWhispAchivAddon", UIParent)
local RegisteredEvents = {}
AutoWhispAchivAddon:SetScript("OnEvent", function (self, event, ...) if (RegisteredEvents[event]) then return RegisteredEvents[event](self, event, ...) end end)

function RegisteredEvents:ADDON_LOADED(event, addon, ...)
	if addon ~= "AutoWhispAchiv" then return end
end
...
for k, v in pairs(RegisteredEvents) do
	AutoWhispAchivAddon:RegisterEvent(k)
end

function AutoWhispAchivDebug(message)
